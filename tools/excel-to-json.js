...
for (const row of rows) {
  ...
  if (!section.teams.has(teamName)) {
    section.teams.set(teamName, {
      code: teamName.toUpperCase().replace(/\s+/g, "_"),
      name: teamName,
      stickers: [],
    });
  }
...
fs.writeFileSync(output, JSON.stringify(album, null, 2), "utf8");

console.log("album.json generated");
console.log("Sections:", album.sections.length);
console.log(
  "Teams:",
  album.sections.reduce((n, s) => n + s.teams.length, 0),
);
